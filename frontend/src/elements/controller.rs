                ...
                Some(d) => {
                    ...
                    if let Some(_) = &self.selected {
                        ...
                        let query = DriveQuery {
                            accelerate: axes[config.drive_axis] * if config.drive_rev { 1.0 } else { -1.0 },
                            steer: axes[config.steer_axis] * if config.steer_rev { 1.0 } else { -1.0 }
                        };

                        //TODO: custom widget?
                        ui.add(egui::ProgressBar::new((query.accelerate + 1.0 / 2.0) as f32).text("drive"));
